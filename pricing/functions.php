 ...
 */
function addPayment($data,$student_id) {
	global $db;
    $trans_date = date('Y-m-d H:i:s');
	if (is_array($data)) {
		$stmt = $db->prepare('INSERT INTO `payments` (student_id, txnid, payment_amount, payment_status, itemid, item_name, payment_currency, receiver_email, payer_email, createdtime) 
		VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
		$stmt->bind_param(
			'ssssssssss',
			$student_id,
		...
		if($data['payment_status'] == 'Completed')
		{
    		try
    		{
              $sql = "UPDATE students SET payment_completed = 1, txnid='".$data['txn_id']."' WHERE student_id = '".$student_id."'";
              $stmt = $db->prepare($sql);
              $stmt->execute();
